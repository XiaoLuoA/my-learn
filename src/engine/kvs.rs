...
/// if let Ok(val) = store.get("key".to_owned()){
...
impl KvStoreWriter {
        ...
        if self.index.contains_key(&key) {
            ...
            if let Command::Remove { key } = cmd {
                if let Some(old_cmd_pos) = self.index.remove(&key) {
                    self.uncompacted += old_cmd_pos.value().len;
    ...
    fn compact(&mut self) -> Result<()> {
        ...
        // compaction_file.lock().unwrap().by_ref().flush()?;
        ...
        for stale_len in stale_gens {
            let file_path = log_path(&self.path, stale_len);
...
impl<P: ThreadPool> KvStore<P> {
    ...
    /// # Errors
    ///
    /// It propagetes I/O or deserialization errors during the log replay.
...
impl<P: ThreadPool> KvsEngine for KvStore<P> {
    ...
    ///
    /// If the key already exists,the previous value will be overwriteen.
    ///
    /// # Errors
    /// It propagetes I/O or deserialization errors during the log replay.
    fn set(
    ...
    /// remove a given key
    /// return an error if the key is not exist or is not removed successfully
            ...
            let res = (|| {
                if let Some(cmd_pos) = index.get(&key) {
                    ...
                    let res = if let Command::Set { value, .. } =
                        reader.read_command(*cmd_pos.value()).unwrap()
                    {
                        Ok(Some(value))
                    } else {
    ...
    while let Some(cmd) = stream.next() {
        ...
        match cmd? {
            ...
            Command::Remove { key } => {
                ...
                // so we add its length to `uncopacted`.
